...
public abstract partial class SharedMartialArtsSystem
{
    ...
    private void OnNinjutsuHug(EntityUid ent, EntityUid target)
    {
        ...
        if (IsBehindTarget(target, ent, Angle.FromDegrees(45d))) // iss14: inline backstab check (BackStabSystem not ported)
        {
    ...
    private void OnNinjutsuAttackPerformed(Entity<NinjutsuSneakAttackComponent> ent, ref AfterComboCheckEvent args)
    {
        ...
        if (args.Performer == args.Weapon)
            _stamina.TakeStaminaDamage(args.Target, 30f); // iss14: resistances apply by default
    ...
    private void OnDirtyKill(Entity<CanPerformComboComponent> ent, ref DirtyKillPerformedEvent args)
    {
        ...
        // iss14: legacy KnockedDown status timing replaced with KnockedDownComponent.NextUpdate.
        if (TryComp<KnockedDownComponent>(target, out var knockedDown))
        {
            var knockdownTime = knockedDown.NextUpdate - _timing.CurTime;
    ...
    private void ResetDebuff(EntityUid uid)
    {
        ...
        // iss14: Goob stealth ninja reveal not ported.
    }
}
