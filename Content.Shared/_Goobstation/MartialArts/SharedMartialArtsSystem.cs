...
// iss14 adaptations:
//  - Changeling / GoobStealth / CustomFactionIcons / heretic hooks stripped (content not present).
//  - BackStabSystem replaced by an inline behind-the-target check.
//  - Legacy "DragonPower" and "LossOfSurprise" status effects kept via legacy status prototypes (alwaysAllowed).
//  - TakeStaminaDamage applyResistances parameter dropped (iss14 applies resistances by default).
using System.Linq;
...
public abstract partial class SharedMartialArtsSystem : EntitySystem
{
    ...
    [Dependency] private Content.Shared.StatusEffect.StatusEffectsSystem _status = default!;
    ...
    [Dependency] private GrabThrownSystem _grabThrowing = default!;
    ...
    protected static readonly EntProtoId ForcedSleepingStatus = "StatusEffectForcedSleeping"; // iss14: RA0033
    protected static readonly EntProtoId BlindnessStatus = "StatusEffectBlindness"; // iss14: RA0033
    ...
    public override void Update(float frameTime)
    {
        ...
        var dragonQuery =
            EntityQueryEnumerator<DragonKungFuTimerComponent, Content.Shared.StatusEffect.StatusEffectsComponent, MobStateComponent, PhysicsComponent>();
    ...
    /// </summary>
    public bool TryGrantMartialArt(EntityUid user, GrantMartialArtKnowledgeComponent comp)
    {
    ...
    private bool GrantMartialArt(GrantMartialArtKnowledgeComponent comp, EntityUid user)
    {
        ...
        var pullerComponent = EnsureComp<Content.Shared.Movement.Pulling.Components.PullerComponent>(user);
    ...
    /// <summary>
    /// iss14: inline replacement for Goob's BackStabSystem.TryBackstab.
    /// Returns true when the attacker is behind the target (within tolerance).
    /// </summary>
    private bool IsBehindTarget(EntityUid target, EntityUid attacker, Angle tolerance)
    {
        var targetRotation = _transform.GetWorldRotation(target).Reduced();
        var dir = _transform.GetWorldPosition(attacker) - _transform.GetWorldPosition(target);
        if (dir.LengthSquared() < 0.001f)
            return false;

        var angleToAttacker = Angle.FromWorldVec(dir).Reduced();
        var diff = Angle.ShortestDistance(targetRotation, angleToAttacker).Reduced();
        return Math.Abs(diff.Theta) >= Math.PI - tolerance.Theta;
    }
