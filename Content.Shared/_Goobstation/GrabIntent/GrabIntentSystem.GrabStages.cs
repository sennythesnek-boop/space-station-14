...
public sealed partial class GrabIntentSystem
{
        ...
        float escapeAttemptModifier = 1f)
    {
        ...
        if (grabIntentComp.GrabStage == GrabStage.Suffocate)
        {
            _stamina.TakeStaminaDamage(pullable.Owner,
                grabIntentComp.SuffocateGrabStaminaDamage); // iss14: resistances apply by default
        ...
        bool dirty = true)
    {
        ...
        // iss14: inlined EinsteinEngines ContestsSystem.MassContest(pullable, puller, bypassClamp: true)
        var massContest = 1f;
        if (TryComp<PhysicsComponent>(pullable.Owner, out var pullablePhysics)
            && TryComp<PhysicsComponent>(puller.Owner, out var pullerPhysics)
            && pullablePhysics.Mass != 0
            && pullerPhysics.InvMass != 0)
        {
            massContest = pullablePhysics.Mass * pullerPhysics.InvMass;
        }

        var massMultiplier = Math.Clamp(massContest * 2f, 0.5f, 2f);
