...
using Content.Shared.Power.Components; // iss14: BatteryComponent is shared now
...
public sealed partial class AugmentPowerCellSystem : SharedAugmentPowerCellSystem
{
    ...
    private void OnBatteryAlert(Entity<HasAugmentPowerCellSlotComponent> ent, ref AugmentBatteryAlertEvent args)
    {
        ...
        var percent = 100f * _battery.GetChargeLevel(battery.AsNullable()); // iss14: charge is inferred, no CurrentCharge field
    ...
    public bool TryUseChargeBody(EntityUid body, float amount)
    {
        ...
        if (!_battery.TryUseCharge(battery.AsNullable(), amount))
        {
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out _))
        {
            ...
            var chargePercent = (short) MathF.Round(_battery.GetChargeLevel(battery.AsNullable()) * 10f);
