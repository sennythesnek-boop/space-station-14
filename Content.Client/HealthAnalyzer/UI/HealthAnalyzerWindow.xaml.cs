...
namespace Content.Client.HealthAnalyzer.UI
{
    ...
    public sealed partial class HealthAnalyzerWindow : FancyWindow
    {
        ...
        public void Populate(HealthAnalyzerBodyMessage msg)
        {
            ...
            var scanned = isPart ? part!.Value : _target.Value;

            ReturnButton.Visible = isPart;
            PartNameLabel.Visible = isPart;
            DamageLabelHeading.Visible = true;
            DamageLabel.Visible = true;
            DamageLabel.Text = _damageable.GetTotalDamage((scanned, damageable)).ToString();
            // Goobstation start
            ...
            var damageSortedGroups =
                _damageable.GetDamagePerGroup((scanned, damageable))
                    .OrderByDescending(damage => damage.Value)
                    .ToDictionary(x => x.Key, x => x.Value);

            var damagePerType = _damageable.GetAllDamage((scanned, damageable)).DamageDict;
...
*/
        ...
        private void CreateOrAddToLayer(Entity<SpriteComponent> sprite, SpriteSpecifier rsi, int layer)
        {
            if (!_spriteSystem.LayerExists((sprite.Owner, sprite.Comp), layer))
                _spriteSystem.AddTextureLayer((sprite.Owner, sprite.Comp), _spriteSystem.Frame0(rsi), layer);
            else
                _spriteSystem.LayerSetTexture((sprite.Owner, sprite.Comp), layer, _spriteSystem.Frame0(rsi));
